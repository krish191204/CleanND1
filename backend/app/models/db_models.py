...
class TweetORM(Base):
    ...
    # Mock-data flag — true for tweets generated by /api/ingest/mock and
    # the kiosk auto-seeder, false for real-API tweets. The dashboard
    # filters these out by default so the live feed never shows
    # fabricated content. Add ?include_mock=true to surface them (e.g.
    # for design review). Added via ALTER TABLE on existing DBs.
